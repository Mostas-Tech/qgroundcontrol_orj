...
void AgriculturalSprayComplexItemTest::_factMetadataAndAreaStates()
{
    ...
    QCOMPARE(dropletClass->metaData()->rawMin().toUInt(), 0U);
    QCOMPARE(dropletClass->metaData()->rawMax().toUInt(), 6U);
    QVERIFY(dropletClass->rawUnits().isEmpty());
    QCOMPARE(dropletClass->enumStrings(),
             QStringList({QStringLiteral("Very Fine"), QStringLiteral("Fine"), QStringLiteral("Medium"),
                          QStringLiteral("Coarse"), QStringLiteral("Very Coarse"), QStringLiteral("Extremely Coarse"),
                          QStringLiteral("Ultra Coarse")}));
    QCOMPARE(dropletClass->enumValues(), QVariantList({0, 1, 2, 3, 4, 5, 6}));

    QGCFencePolygon* const invalid = _addPolygon(true);
    QVERIFY(invalid);
...
void AgriculturalSprayComplexItemTest::_fenceEditsRebuildWhileDirty()
{
    ...
    QGCFencePolygon* const inclusionPolygon = geoFenceController()->polygons()->value<QGCFencePolygon*>(0);
    QVERIFY(inclusionPolygon);
    inclusionPolygon->setPath(square(center, 120.0));
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != initialRoute,
        TestTimeout::shortMs());
    const QList<QGeoCoordinate> firstPolygonEditRoute = _item->routeCoordinates();

    inclusionPolygon->setPath(square(center, 80.0));
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != firstPolygonEditRoute,
        TestTimeout::shortMs());
    QVERIFY(_item->dirty());

    inclusionPolygon->setInclusion(false);
    _waitForStatus(AgriculturalSprayComplexItem::NoInclusion);
    geoFenceController()->deletePolygon(0);

    QGCFenceCircle* const inclusionCircle = _addCircle(true);
    QVERIFY(inclusionCircle);
    inclusionCircle->setCenter(center);
    inclusionCircle->radius()->setRawValue(45.0);
    _waitForStatus(AgriculturalSprayComplexItem::Ready);
    const QList<QGeoCoordinate> initialCircleRoute = _item->routeCoordinates();

    inclusionCircle->setCenter(center.atDistanceAndAzimuth(10.0, 90.0));
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != initialCircleRoute,
        TestTimeout::shortMs());
    const QList<QGeoCoordinate> firstCircleEditRoute = _item->routeCoordinates();

    inclusionCircle->radius()->setRawValue(35.0);
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != firstCircleEditRoute,
        TestTimeout::shortMs());
    QVERIFY(_item->dirty());

    inclusionCircle->setInclusion(false);
    _waitForStatus(AgriculturalSprayComplexItem::NoInclusion);
    geoFenceController()->deleteCircle(0);
    QGCFencePolygon* const replacementInclusion = _addPolygon(true);
    QVERIFY(replacementInclusion);
    replacementInclusion->setPath(square(center, 80.0));
    _waitForStatus(AgriculturalSprayComplexItem::Ready);
    const QList<QGeoCoordinate> inclusionRoute = _item->routeCoordinates();

    QGCFencePolygon* const exclusion = _addPolygon(false);
    QVERIFY(exclusion);
    exclusion->setPath(square(center, 20.0));
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != inclusionRoute,
        TestTimeout::shortMs());
    QVERIFY(_item->dirty());

    const QList<QGeoCoordinate> polygonExcludedRoute = _item->routeCoordinates();
    QGCFenceCircle* const circle = _addCircle(false);
    QVERIFY(circle);
    circle->setCenter(center.atDistanceAndAzimuth(18.0, 90.0));
    circle->radius()->setRawValue(5.0);
    QVERIFY_TRUE_WAIT(
        _item->status() == AgriculturalSprayComplexItem::Ready && _item->routeCoordinates() != polygonExcludedRoute,
        TestTimeout::shortMs());
    QVERIFY(_item->dirty());

    const QList<QGeoCoordinate> firstExclusionCircleRoute = _item->routeCoordinates();
    circle->radius()->setRawValue(10.0);
    QVERIFY_TRUE_WAIT(_item->status() == AgriculturalSprayComplexItem::Ready &&
                          _item->routeCoordinates() != firstExclusionCircleRoute,
                      TestTimeout::shortMs());
}
...
void AgriculturalSprayComplexItemTest::_planningInputsAndMetadataOnlyInputs()
{
    ...
    _item->dropletClass()->setRawValue(AgriculturalSprayComplexItem::UltraCoarse);
    _waitForStatus(AgriculturalSprayComplexItem::Ready);
...
void AgriculturalSprayComplexItemTest::_sequenceAndDownstreamRecalculation()
{
    AgriculturalSprayComplexItem* const spray =
        qobject_cast<AgriculturalSprayComplexItem*>(missionController()->insertComplexMissionItem(
            AgriculturalSprayComplexItem::canonicalName, QGeoCoordinate(47.397742, 8.545594), -1, false));
    QVERIFY(spray);
    VisualMissionItem* const downstream =
        missionController()->insertSimpleMissionItem(QGeoCoordinate(47.398000, 8.546000), -1);
    QVERIFY(downstream);
    QCOMPARE(spray->sequenceNumber(), 1);
    QCOMPARE(downstream->sequenceNumber(), spray->lastSequenceNumber() + 1);

    _makeReadyWithSquare();
    QCOMPARE_TRUE_WAIT(spray->status(), AgriculturalSprayComplexItem::Ready, TestTimeout::shortMs());
    QVERIFY_TRUE_WAIT(downstream->sequenceNumber() == spray->lastSequenceNumber() + 1, TestTimeout::shortMs());
    const int initialLastSequenceNumber = spray->lastSequenceNumber();
    const int initialDownstreamSequenceNumber = downstream->sequenceNumber();

    spray->lineSpacing()->setRawValue(25.0);
    QVERIFY_TRUE_WAIT(spray->status() == AgriculturalSprayComplexItem::Ready &&
                          spray->lastSequenceNumber() != initialLastSequenceNumber &&
                          downstream->sequenceNumber() == spray->lastSequenceNumber() + 1,
                      TestTimeout::shortMs());
    QVERIFY(downstream->sequenceNumber() != initialDownstreamSequenceNumber);
}
